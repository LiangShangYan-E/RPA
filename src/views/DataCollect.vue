  ...
  <div class="data-view">
    ...
    <el-card shadow="never" class="content-card">
      ...
      <el-table :data="pagedRows" stripe class="data-table">
        ...
        <el-table-column prop="source" label="数据来源" min-width="160" show-overflow-tooltip />
        <el-table-column prop="collectTime" label="采集时间" width="180" />
        <el-table-column label="操作" width="140" fixed="right" align="center">
          <template #default="{ row }">
            <el-button type="primary" link @click="goParse(row.collectId)">查看</el-button>
...
const filteredRows = computed(() => {
  ...
  return allRows.value.filter((r) => {
    ...
    if (!start || !end) return okTask && okKeyword && okStatus
    const ct = parseDateTime(r.collectTime)
    if (!ct) return okTask && okKeyword && okStatus
    const time = ct.getTime()
    return okTask && okKeyword && okStatus && time >= start.getTime() && time <= end.getTime()
  })
})
...
const loadRows = () => {
  ensureSeeded()
  allRows.value = rpaStore.collects
}
...
const goParse = (collectId) => {
  router.push({ path: '/data/parse', query: { collectId } })
}

const onDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除该记录吗？', '提示', { type: 'warning', confirmButtonText: '确定', cancelButtonText: '取消' })
    const idx = rpaStore.collects.findIndex((r) => r.collectId === row.collectId)
    if (idx >= 0) rpaStore.collects.splice(idx, 1)
    ElMessage.success('删除成功')
  } catch {}
}

const onCreate = () => {
  ElMessage.info('新增功能待接入后端')
}
...
watch(
  () => [route.query.taskCode, route.query.collectId],
  () => applyQuery()
)
