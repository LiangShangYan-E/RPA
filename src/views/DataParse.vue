  ...
  <div class="data-view">
    ...
    <el-card shadow="never" class="content-card">
      ...
      <el-table :data="pagedRows" stripe class="data-table">
        ...
        <el-table-column prop="collectId" label="采集ID" min-width="180">
          <template #default="{ row }">
            <el-link type="primary" :underline="false" @click="goCollect(row.collectId)">{{ row.collectId }}</el-link>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="fieldCount" label="提取字段数" width="120" align="center" />
        <el-table-column prop="ruleApplied" label="解析规则" width="120" align="center">
          <template #default="{ row }">
            <el-checkbox v-model="row.ruleApplied" disabled />
          </template>
        </el-table-column>
        <el-table-column prop="parseTime" label="解析时间" width="180" />
        <el-table-column label="操作" width="140" fixed="right" align="center">
          <template #default="{ row }">
            <el-button type="primary" link @click="goProcess(row.parseId)">查看</el-button>
...
const filteredRows = computed(() => {
  ...
  return allRows.value.filter((r) => {
    ...
    const ct = parseDateTime(r.parseTime)
...
const goCollect = (collectId) => {
  router.push({ path: '/data/collect', query: { collectId } })
}

const goProcess = (parseId) => {
  router.push({ path: '/data/process', query: { parseId } })
}

const onDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除该记录吗？', '提示', { type: 'warning', confirmButtonText: '确定', cancelButtonText: '取消' })
    const idx = rpaStore.parses.findIndex((r) => r.parseId === row.parseId)
    if (idx >= 0) rpaStore.parses.splice(idx, 1)
...
const applyQuery = () => {
  ...
  const collectId = typeof route.query.collectId === 'string' ? route.query.collectId : ''
  ...
  if (collectId) {
    const row = allRows.value.find((r) => r.collectId === collectId)
    if (row?.taskCode) {
      searchForm.taskCode = row.taskCode
      appliedSearch.taskCode = row.taskCode
      currentPage.value = 1
...
watch(
  () => [route.query.taskCode, route.query.collectId, route.query.parseId],
  () => applyQuery()
)
